...
\paragraph{Variants compared.} Three Treeffuser rows attribute contributions cleanly: (1)~\textsc{published}---the original baseline with noise-prediction, uniform $t$, no residualization; (2)~\textsc{score+}---the baseline plus \S\ref{sec:score-side}; (3)~\textsc{FM}---VP flow matching with residualizer-C and a 5-step Heun ODE. All share the same LightGBM size. We also compare against five tuned probabilistic baselines: NGBoost~\citep{duan2020ngboost}, quantile-regression LightGBM~\citep{ke2017lightgbm}, CatBoost RMSE-with-uncertainty~\citep{prokhorenkova2018catboost}, iBUG~\citep{brophy2022ibug}, and a Gaussian deep ensemble~\citep{lakshminarayanan2017deep}.
...
\paragraph{Limitations.} External baselines use fixed family-level grids selected on a small validation suite, not per-dataset Bayesian optimization. We include NGBoost, iBUG, CatBoost uncertainty, quantile-regression LightGBM, and a deep ensemble, but not CARD~\citep{han2022card} or distributional random forests~\citep{cevid2022drf}. More importantly, the residualizer and score+/FM hyperparameters are fixed by sweeps run mostly on small-to-mid datasets and reused across the suite; the large-dataset inversions in Table~\ref{tab:per-dataset} identify per-dataset residualizer and time-sampling selection as the main remaining tuning axis.
...
\paragraph{Variance-preserving paths stabilize tree features.} After target standardization and ignoring conditional mean for scale intuition, $\Var(y_0)\approx\Var(z)\approx 1$ and $y_0\perp z$. Then $\Var(y_t)\approx\alpha(t)^2+\beta(t)^2$. Trigonometric and VP paths keep this quantity approximately constant, whereas the linear path gives $(1-t)^2+t^2$, with minimum $1/2$ at $t=1/2$. For histogram-split trees, this matters because the empirical distribution of $y_t$ determines both candidate split locations and where leaf resolution is spent.
...
\paragraph{Endpoint amplification.} Eq.~\eqref{eq:score} contains a $1/\beta(t)$ factor. Near the data endpoint, score error induced by velocity error is therefore amplified by approximately $1/\beta(t)$. Schedules with $\varepsilon(t)\to 0$ at the endpoint avoid injecting this high-gain approximate score into the diffusion term; the deterministic ODE ($\varepsilon\equiv0$) is the limiting case.
...
We compare the headline Treeffuser variants against five external probabilistic regressors (Table~\ref{tab:competitors}). Hyperparameters for each baseline were selected by a per-model grid summarised in this appendix's accompanying configs (\texttt{benchmarks/configs/probabilistic\_baseline\_hyperparams/}).
...
The large-dataset inversion in Table~\ref{tab:per-dataset} admits two plausible mechanisms: the fixed residualizer may be suboptimal at high $n$, and stochastic samplers may improve conditional tail calibration in high-uncertainty regions. The benchmark logs already include coverage stratified by predicted-IQR quintile, allowing a lightweight diagnostic without rerunning. Table~\ref{tab:tail-diagnostic} reports the absolute 95\% coverage error in the top predicted-IQR bin on \texttt{california\_housing} and \texttt{protein}.

\begin{table}[!h]
  \centering
  \caption{High-uncertainty tail diagnostic on the two largest datasets. ``Top-IQR'' is $|\text{cE}|$@95 in the highest predicted-IQR quintile; IQR-MACE is mean $|\text{cE}|$@95 across predicted-IQR quintiles. Lower is better. Full rows use the full-data headline run; subset rows use the four-dataset residualizer-FM sweep.}\label{tab:tail-diagnostic}
  \scriptsize
  \begin{tabular}{lllrrrr}
    \toprule
    Dataset & Run & Variant & CRPS & $|\text{cE}|$@95 & IQR-MACE & Top-IQR \\
    \midrule
    cali.\ housing & full & Published SDE & \textbf{0.207} & 0.009 & \textbf{0.015} & \textbf{0.019} \\
                  & full & FM ODE-C      & 0.214 & \textbf{0.003} & 0.021 & 0.050 \\
                  & full & FM ODE-E      & 0.213 & \textbf{0.003} & 0.021 & 0.050 \\
                  & subset & FM ODE-C    & \textbf{0.225} & \textbf{0.006} & 0.020 & 0.042 \\
                  & subset & FM SDE-C    & 0.227 & 0.016 & 0.022 & 0.012 \\
                  & subset & FM SDE-C $\sqrt{t}$ & 0.227 & 0.015 & \textbf{0.020} & \textbf{0.010} \\
    protein       & full & Published SDE & 1.787 & 0.006 & 0.018 & 0.025 \\
                  & full & FM ODE-C      & 1.860 & 0.017 & 0.025 & 0.027 \\
                  & full & FM ODE-E      & \textbf{1.771} & \textbf{0.002} & \textbf{0.008} & \textbf{0.007} \\
                  & subset & FM ODE-E    & \textbf{2.223} & \textbf{0.008} & 0.014 & 0.023 \\
                  & subset & FM SDE-E    & 2.228 & 0.014 & 0.016 & 0.010 \\
                  & subset & FM SDE-E $\sqrt{t}$ & 2.225 & 0.012 & \textbf{0.012} & \textbf{0.008} \\
    \bottomrule
  \end{tabular}
\end{table}

The diagnostic supports a tradeoff rather than a one-sided conclusion. On \texttt{california\_housing}, the published SDE has much better high-IQR tail coverage than the full-data FM ODE rows, and the matched subset sweep shows that adding FM stochasticity closes the same top-IQR gap. On \texttt{protein}, however, the full-data FM ODE-E row already has the best CRPS and top-IQR coverage, while stochastic subset rows improve top-IQR coverage relative to ODE-E only by giving up aggregate CRPS and marginal 95\% coverage. Thus the safest interpretation is not that SDE samplers are dominated, but that their benefit is conditional and must be selected against conditional-calibration metrics rather than headline CRPS alone.

\section{Reproducibility}\label{app:repro}

All sweeps in this paper are driven by YAML configs under \texttt{benchmarks/configs/}. Each result file is a JSON Lines document recording the full hyperparameter set, the random seeds (data/model/sampler), the git commit hash of the model code, a hash of the Treeffuser source tree, and every reported metric on a per-(dataset, seed) row. The full-data runs reported in the main paper used commit \texttt{99e3665} of the model source (\texttt{git\_sha} field on every row of \texttt{paper\_real\_data\_v2\_full.jsonl} and \texttt{paper\_probabilistic\_baselines\_selected\_full.jsonl}); the configs that produced the appendix tables are listed alongside in Table~\ref{tab:appendix-provenance}.
...
\begin{table}[!h]
  ...
  \begin{tabular}{lll}
    ...
    \ref{tab:competitors}   & \texttt{paper\_probabilistic\_baselines\_selected.yaml} & \texttt{paper\_probabilistic\_baselines\_selected\_full.jsonl} \\
    ...
    \ref{tab:tail-diagnostic} & \texttt{paper\_real\_data\_v2.yaml}, \texttt{residualizer\_fm\_sweep.yaml} & \texttt{paper\_real\_data\_v2\_full.jsonl}, \texttt{residualizer\_fm\_sweep\_\_*.jsonl} \\
    \bottomrule
  \end{tabular}}
\end{table}
